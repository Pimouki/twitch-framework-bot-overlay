...
let messageCount = 0;
async function appInit([{ eventSub, chatClient: chatBroadcasterClient, apiClient, pubSubClient }, { chatClient: chatBotClient, apiClient: apiBotClient }, obs, spotify,]) {
    ...
    chatBotClient.onMessage(async (channel, user, text, meta) => {
        ...
        if (usersBlacklist.includes(userLower))
            return;
        const parsedText = (0, chat_1.extractMessageText)(meta.text);
    ...
    eventSub.onChannelRedemptionAdd(configs_1.TWITCH_BROADCASTER_ID, async (message) => {
