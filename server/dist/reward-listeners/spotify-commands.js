...
const SpotifyRewardCommands = async ({ channel, rewardTitle, rewardId, user, userId, message, chatBotClient, chatBroadcasterClient, apiClient, apiBotClient, socket, obs, spotify, }) => {
    ...
    try {
        ...
        chatBotClient.say(channel, `Je vais jouer la troubadour :  ${response.artists[0].name} - ${response.name}`);
    }
    catch (e) {
        chatBotClient.say(channel, "La troubadour que je suis ne a cassée une corde a son lute");
        console.warn(e);
    }
    finally {
        return true;
...
exports.SpotifyRewardCommands = SpotifyRewardCommands;
